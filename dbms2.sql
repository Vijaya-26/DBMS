create database vijbanking;
use vijbanking;
CREATE TABLE branch(branch_name char(30), branch_city char(30), assets real, PRIMARY KEY(branch_name));
CREATE TABLE bank_account(accno int, branch_name char(30), balance real, PRIMARY KEY(accno), FOREIGN KEY(branch_name) REFERENCES branch(branch_name) on delete cascade);
...
CREATE TABLE loan(loan_number int, branch_name char(30), amount real, PRIMARY KEY(loan_number), FOREIGN KEY(branch_name) REFERENCES branch(branch_name) on delete cascade);
...
SELECT customer_name FROM depositor d, bank_account a where d.accno=a.accno AND a.branch_name="SBI_Basvangudi" GROUP BY d.customer_name HAVING COUNT(d.customer_name)>=2;
...
DELETE FROM bank_account WHERE branch_name IN(SELECT branch_name FROM branch WHERE branch_city='Vellore');
